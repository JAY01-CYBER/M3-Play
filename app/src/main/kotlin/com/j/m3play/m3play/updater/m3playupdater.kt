...
package com.j.m3play.M3-Play.updater
...
import com.j.m3play.M3-Play.updater.downloadmanager.UpdateDownloadWorker
import com.j.m3play.M3-Play.updater.downloadmanager.DownloadNotificationManager
...
import com.j.m3play.ui.component.AnimatedActionButton
import com.j.m3play.ui.component.ExpressiveIconButton
import com.j.m3play.ui.component.ErrorSnackbar
...
sealed class M3PlayUpdateStatus {
    object Idle : M3PlayUpdateStatus()
    object Checking : M3PlayUpdateStatus()
    ...
    data class NoUpdate(val version: String) : M3PlayUpdateStatus()
    data class Error(val message: String) : M3PlayUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<M3PlayUpdateStatus>(M3PlayUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = M3PlayUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        M3PlayUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = M3PlayUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    Text(
                        text = if (status is M3PlayUpdateStatus.Available) stringResource(R.string.new_update) else stringResource(R.string.settings_check_updates_title),
                        maxLines = 1
                    )
                },
                navigationIcon = {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is M3PlayUpdateStatus.Idle, is M3PlayUpdateStatus.Checking, is M3PlayUpdateStatus.NoUpdate, is M3PlayUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is M3PlayUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is M3PlayUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/m3playzzz007/m3-play/releases/download/${currentStatus.version}/m3-play.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is M3PlayUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is M3PlayUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is M3PlayUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/m3playzzz007/m3-play/releases")
            ...
            if (targetRelease != null) {
                ...
                if (shouldShow) {
                    ...
                    try {
                        val changelogUrl =
                            URL("https://github.com/m3playzzz007/m3-play/releases/download/$tagWithPrefix/changelog.json")
