...
data class HomeShelf(val title: String, val items: List<YTItem>)

suspend fun YouTube.getExactMusicHome(): Result<List<HomeShelf>> = withContext(Dispatchers.IO) {
    runCatching {
                ...
                carouselRenderer.contents?.forEach { itemRenderer ->
                    ...
                    if (twoRowItem != null) {
                        ...
                        when {
                            ...
                            browseId?.startsWith("MPREb") == true -> items.add(
                                AlbumItem(
                                    browseId = browseId,
                            ...
                            browseId?.startsWith("VL") == true || browseId?.startsWith("PL") == true -> items.add(
                                PlaylistItem(
                            ...
                            browseId?.startsWith("UC") == true -> items.add(
                                ArtistItem(
                                    id = browseId,
                                    title = itemTitle,
                                    thumbnail = thumbnail,
